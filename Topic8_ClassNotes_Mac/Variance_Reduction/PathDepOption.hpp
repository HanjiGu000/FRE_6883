...
namespace fre {
    ...
    // combines two options with payoffs H(T) and G(T),
    // and creates an option with payoff H(T)-G(T).
    // Pointers Ptr1 and Ptr2 refer to options with payoffs
    // H(T) and G(T). The payoff is equal to H(T)-G(T).
